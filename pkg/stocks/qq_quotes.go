...
// 获取最新行情 http://qt.gtimg.cn/q=sh600519
// 获取实时资金流向 http://qt.gtimg.cn/q=ff_sh600519
// 获取盘口分析 http://qt.gtimg.cn/q=s_pksh600519
// 获取简要信息 http://qt.gtimg.cn/q=s_sh600519

const quotesURLQQ = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=%s`
const quotesURLQQQueryParts = ``
...
type Quotes struct {
	market  *TerminalStocks.Market  // Pointer to Market.
	profile *TerminalStocks.Profile // Pointer to Profile.
...
func NewQuotes(market *TerminalStocks.Market, profile *TerminalStocks.Profile) *Quotes {
	return &Quotes{
		market:  market,
		profile: profile,
		errors:  ``,
...
func (quotes *Quotes) Fetch() (self *Quotes) {
	...
	if quotes.isReady() {
		...
		url := fmt.Sprintf(quotesURLQQ, strings.Join(quotes.profile.Tickers, `,`))
		response, err := http.Get(url + quotesURLQQQueryParts)
		if err != nil {
			panic(err)
		}
